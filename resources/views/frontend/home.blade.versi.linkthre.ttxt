...
<body class="antialiased">
  ...
  <!-- Admin Dashboard Button (hanya tampil jika admin login) -->
  @if(auth()->guard('admin')->check())
    <div class="fixed top-4 right-4 z-50">
      <a href="{{ route('admin.dashboard') }}"
        class="glass rounded-2xl p-3 hover:bg-white/20 transition-all duration-300 transform hover:scale-110 shadow-lg group"
        title="Kembali ke Dashboard Admin">
        <i class="fas fa-tachometer-alt text-cyan-300 group-hover:text-cyan-100 transition-colors text-lg"></i>
      </a>
    </div>
  @endif
