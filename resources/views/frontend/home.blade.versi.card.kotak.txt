  ...
  < <!-- Navbar -->
      ...
      <section id="cards-container"
        ...
        @forelse ($cards as $card)
          <article tabindex="0" data-aos="fade-up"
            class="bg-white rounded-xl shadow-lg border border-gray-100 hover:shadow-2xl hover:shadow-emerald-500/20 hover:-translate-y-1 transition-all duration-300 ease-out flex flex-col overflow-hidden cursor-pointer group">
            <div class="w-full h-32 sm:h-40 overflow-hidden relative">
              <img src="{{ Storage::url($card->image_url) }}" alt="Logo {{ $card->title }}"
                class="w-full h-full object-cover bg-gray-100 transition-transform duration-300 group-hover:scale-110" />
    ...
    <script>
      ...
      function loadInitialCards() {
        // Logic to reload the initial cards can be added here
      }
