...
@section('content')
<div class="p-6 transition-all duration-300" x-data="{ ...userManager(), sidebarOpen: sidebarOpen }" x-cloak @keydown.escape.window="closeAllModals()">

<h1 class="text-2xl font-bold mb-6 text-emerald-900">Manajemen Pengguna Admin</h1>

<!-- Button Tambah Pengguna -->
<button @click="openCreateModal = true" class="mb-4 bg-emerald-700 hover:bg-emerald-800 text-white px-4 py-2 rounded shadow focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2">
...
<table class="w-full border border-emerald-300 rounded-lg overflow-hidden">
    ...
    <tbody>
      ...
      <tr class="border-b border-emerald-300 hover:bg-emerald-50">
        ...
        <td class="p-3 text-center space-x-2">
          <button @click="openEditModal({{ $user->id }})" class="text-emerald-600 hover:text-emerald-800" title="Edit">
            <!-- SVG icon edit -->
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
              <path d="M12 20h9" />
              <path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z" />
            </svg>
          </button>
          <button @click="openDeleteModal({{ $user->id }})" class="text-red-600 hover:text-red-800" title="Hapus">
            <!-- SVG icon trash -->
            <svg xmlns="http://www.w3.org/2000/svg" class="inline-block w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
  ...
  <!-- Modal Create -->
  <div x-cloak>
    <div x-show="openCreateModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div @click.away="openCreateModal = false" x-transition class="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl space-y-4">
        ...
        <form x-ref="createForm" @submit.prevent="submitCreateForm" class="space-y-4">
          ...
          <div class="flex justify-end space-x-3 pt-2">
            <button type="button" @click="openCreateModal = false" class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 shadow">Batal</button>
            <button type="submit" class="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white" :disabled="loading">Simpan</button>
          </div>
        </form>
      </div>
    </div>
  ...
  <!-- Edit Modal -->
  <div x-show="openEditModalId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div @click.away="openEditModalId = null" x-transition class="bg-white rounded-xl p-6 w-full max-w-lg shadow-xl space-y-4">
      <h2 class="text-2xl font-bold text-emerald-700">Edit Pengguna Admin</h2>

      <form :action="`{{ url('admin/users') }}/${openEditModalId}`" method="POST" x-ref="editForm" @submit.prevent="submitEditForm" class="space-y-4" novalidate>
        @csrf
        @method('PUT')
        ...
        <!-- Tombol -->
        <div class="flex justify-end space-x-3 pt-2">
          <button type="button" @click="openEditModalId = null" class="px-4 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 text-sm text-gray-800 shadow">Batal</button>
          <button type="submit" class="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white" :disabled="loading">Simpan</button>
        </div>
      </form>
    </div>
  ...
  <!-- Delete Modal -->
  <div x-show="openDeleteModalId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div @click.away="openDeleteModalId = null" class="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl ring-1 ring-black ring-opacity-5" style="z-index: 9999;">
      <h2 class="text-xl font-semibold mb-4">Hapus Pengguna Admin</h2>
      <p>Apakah Anda yakin ingin menghapus pengguna admin ini?</p>
      <form :action="`{{ url('admin/users') }}/${openDeleteModalId}`" method="POST" class="mt-4" @submit.prevent="submitDeleteForm">
        @csrf
        @method('DELETE')
        <div class="flex justify-end space-x-3">
          <button type="button" @click="openDeleteModalId = null" class="px-4 py-2 rounded bg-gray-300 hover:bg-gray-400" tabindex="0">Batal</button>
          <button type="submit" class="px-4 py-2 rounded bg-red-600 text-white hover:bg-red-700" tabindex="0">Hapus</button>
        </div>
      </form>
    </div>
  </div>
...
<div x-show="openValidationModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
  <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
    ...
    <div class="flex justify-end mt-4">
      <button @click="closeValidationModal" class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">OK</button>
    </div>
  </div>
</div>
...
<div x-show="openSuccessModal" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
  <div class="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
    ...
    <div class="flex justify-end mt-4">
      <button @click="closeSuccessModal" class="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700">OK</button>
    </div>
  </div>
</div>
