...
<body class="bg-gray-100 text-gray-800 flex" x-data="{ sidebarOpen: true }">
  ...
  <div :class="sidebarOpen ? 'ml-64' : 'ml-16'" class="flex-1 flex flex-col min-h-screen transition-all duration-300">
    ...
    <!-- Navbar -->
    <nav class="bg-gradient-to-r from-emerald-900 via-emerald-800 to-emerald-700 text-white px-6 py-6 shadow-md flex justify-between items-center">
      ...
      <div x-data="{ open: false }" class="relative">
        ...
        <div x-show="open" @click.outside="open = false" class="absolute right-0 mt-2 w-48 bg-white text-gray-700 rounded-md shadow-lg py-2 z-50">
    ...
    <main class="p-6 flex-1 overflow-auto">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <!-- Manajemen Kartu -->
        <a href="{{ route('admin.cards.index') }}" class="group bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition transform hover:-translate-y-1">
        ...
        <!-- Manajemen Pengguna -->
        <a href="{{ route('admin.users.index') }}" class="group bg-white rounded-xl shadow-md p-6 hover:shadow-lg transition transform hover:-translate-y-1">
          <div class="flex items-center gap-4 mb-3">
            <div class="bg-emerald-100 text-emerald-800 p-3 rounded-full">
              <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
