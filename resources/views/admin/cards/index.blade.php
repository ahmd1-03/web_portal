...
@section('content')
<div class="p-6 transition-all duration-300" x-data="{ ...cardManager(), sidebarOpen: sidebarOpen }" x-cloak @keydown.escape.window="closeAllModals()">

<!-- Add missing Alpine.js directives for modal open states and validation/success modal close methods -->
<h1 class="text-2xl font-bold mb-6 text-emerald-900">Manajemen Kartu Konten</h1>
...
<form x-ref="createForm" @submit.prevent="submitCreateForm" enctype="multipart/form-data" class="space-y-4">
      ...
      <div x-data="{ preview: null }">
        ...
        <template x-if="preview">
          <img :src="preview" alt="Preview Gambar" class="mt-4 h-32 rounded-lg shadow object-contain border border-gray-200" />
        </template>
      </div>
...
<div x-show="openEditModalId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
  <div @click.away="openEditModalId = null" x-transition
    ...
    <form :action="`{{ url('admin/cards') }}/${openEditModalId}`" method="POST" x-ref="editForm"
      ...
      <!-- Link Eksternal -->
      <div>
        <label for="edit_external_link" class="block text-sm font-medium text-emerald-800 mb-1">Link Eksternal</label>
        <input type="url" name="external_link" id="edit_external_link" required x-model="cardData.external_link"
          class="w-full bg-emerald-50 border border-emerald-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-emerald-400 focus:outline-none shadow-sm placeholder:text-emerald-500" />
      </div>
...
<div x-show="openDeleteModalId !== null" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
  <div @click.away="openDeleteModalId = null" class="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl ring-1 ring-black ring-opacity-5" style="z-index: 9999;">
    ...
    <form :action="`{{ url('admin/cards') }}/${openDeleteModalId}`" method="POST" class="mt-4" @submit.prevent="submitDeleteForm">
      @csrf
      @method('DELETE')
